...
class Cache
{
    ...
    {
        $this->config = $config;
        $this->compressLevel = $config->getCompressLevel();
        $this->keySlotsMem = $config->getKeySlotsMem();
        $this->maxCacheMem = $config->getMaxCacheMem();
        $this->cacheSlotsMem = $config->getCacheSlotsMem();
        $this->maxQueueMem = $config->getMaxQueueMem();
        $this->storage = $config->getStorageInterface();
        $this->cacheDir = $config->getCacheDir();
        if (substr($this->cacheDir, -1) != '/') {
            $this->cacheDir .= '/';
        }
        if (!is_dir($this->cacheDir) && !mkdir($this->cacheDir, 0744)) {
            throw new \Exception("{$this->cacheDir} create fail");
        }
        ...
        if ($keyMem->open($this->cacheDir . 'keySlots', $this->keySlotsMem)) {
            ...
            throw new \Exception('key slots memory open failed');
        }
    }
    ...
    private function getCacheMem(int $hash) : ?Storage\StorageInterface
    {
        ...
        if (!isset($this->cacheMem[$cacheSlot])) {
            ...
            if ($cacheMem->open($dir . 'cache_' . $cacheSlot, $this->cacheSlotsMem) === false) {
                throw new \Exception('open cache memory failed');
            }
    ...
    private function getQueueMem(string $queue) : ?Storage\StorageInterface
    {
        if (isset($this->queueMem[$queue])) {
            ...
            if ($queueMem->open($dir . $queue, $this->maxQueueMem) === false) {
                throw new \Exception('open queue memory failed');
            }
            /**
    ...
    {
        if ($this->storage != 'MMap') {
            throw new \Exception('only mmap has flush method');
        }
        $this->_flush();
    }
    ...
    {
        if ($this->storage != 'ShMop') {
            throw new \Exception('only shmop has delete method');
        }
        $this->_delete();
    }
}